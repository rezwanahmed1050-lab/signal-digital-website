...
function submitForm(formId, successId, btnId) {
  ...
  if (window.emailjs) {
    /* Prepare form data with exact template variable names */
    ...
    emailjs.send('service_gen2xxn', 'template_fvi10vn', templateParams)
      .then(response => {
        ...
        return emailjs.send('service_gen2xxn', 'template_y1shs49', templateParams);
      })
      .then(response => {
        console.log('Client auto-reply sent successfully:', response);
    ...
    if (btn) { btn.disabled = false; btn.innerHTML = 'Send Message →'; }
    alert('Email system not initialized. Please try again or email us directly.');
  }
}
